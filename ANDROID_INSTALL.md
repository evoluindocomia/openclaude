# FreeCoding on Android (Termux)

A complete guide to running FreeCoding on Android using Termux + proot Ubuntu.
...
FreeCoding requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 3 — Clone FreeCoding

```bash
git clone https://github.com/evoluindocomia/affhub-freecoding.git
cd freecoding
npm install
npm link
```
...
### Step 6 — Build FreeCoding

```bash
cd /data/data/com.termux/files/home/freecoding
bun run build
```

You should see:
```
✓ Built freecoding v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to FreeCoding's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the FreeCoding repo.
