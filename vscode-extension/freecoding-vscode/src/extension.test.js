...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'freecoding',
    launchCommand: 'freecoding --project-aware',
    terminalName: 'FreeCoding',
    shimEnabled: false,
    workspaceFolder: '/workspace/freecoding/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/freecoding/very/long/path/example-project',
    launchCwdLabel: '/workspace/freecoding/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/freecoding/very/long/path/example-project/.freecoding-profile.json',
    workspaceProfilePath: '/workspace/freecoding/very/long/path/example-project/.freecoding-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the FreeCoding wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/freecoding\/very\/long\/path\/example-project"[^>]*>\/workspace\/freecoding\/very\/long\/path\/example-project<\//,
  );
});
...
test('FreeCodingControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { FreeCodingControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/freecoding/src/panels/control-center.js',
      workspacePath: '/workspace/freecoding',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/freecoding/src/panels',
      projectAwareCwdLabel: '/workspace/freecoding/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/freecoding',
      workspaceRootCwdLabel: '/workspace/freecoding',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/freecoding',
      activeFilePath: '/workspace/freecoding/src/panels/control-center.js',
      workspacePath: '/workspace/freecoding',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/freecoding',
      projectAwareCwdLabel: '/workspace/freecoding',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/freecoding/src/panels',
      launchCwdLabel: '/workspace/freecoding/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/freecoding\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/freecoding/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/freecoding',
      launchCwdLabel: '/workspace/freecoding',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/freecoding/);
  assert.match(html, /Same workspace-root target as Launch FreeCoding because the relative command resolves from the workspace root · \/workspace\/freecoding/);
});
