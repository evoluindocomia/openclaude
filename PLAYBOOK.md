# FreeCoding Local Agent Playbook

This playbook is a practical guide to run FreeCoding with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, FreeCoding starts directly.
...
Or let FreeCoding recommend the best local model for your goal:
...
```powershell
Get-Content .\.freecoding-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\freecoding\freecoding
bun run dev:profile
```
...
- Keep `.freecoding-profile.json` local (already gitignored).
