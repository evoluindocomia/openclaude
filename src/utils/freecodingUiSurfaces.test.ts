...
describe('FreeCoding settings path surfaces', () => {
  test('isClaudeSettingsPath recognizes project .freecoding settings files', () => {
    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.freecoding', 'settings.json'),
      ),
    ).toBe(true)

    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.freecoding', 'settings.local.json'),
      ),
    ).toBe(true)
  })

  test('permission save destinations point user settings to ~/.freecoding', () => {
  ...
  test('permission save destinations point project settings to .freecoding', () => {
    expect(optionForPermissionSaveDestination('projectSettings')).toEqual({
      label: 'Project settings',
      description: 'Checked in at .freecoding/settings.json',
      value: 'projectSettings',
    })

    expect(optionForPermissionSaveDestination('localSettings')).toEqual({
      label: 'Project settings (local)',
      description: 'Saved in .freecoding/settings.local.json',
      value: 'localSettings',
    })
  })
})

describe('FreeCoding validation tips', () => {
