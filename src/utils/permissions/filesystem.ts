...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.freecoding${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.freecoding${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.freecoding', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.freecoding', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.freecoding', 'skills')
