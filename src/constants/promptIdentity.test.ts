...
;(globalThis as Record<string, unknown>).MACRO = {
  ...
  PACKAGE_URL: '@evoluindocomia/affhub-freecoding',
...
test('CLI identity prefixes describe FreeCoding instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('FreeCoding')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('FreeCoding')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes FreeCoding instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('FreeCoding')
  expect(prompt[0]).not.toContain('Claude Code')
  expect(prompt[0]).not.toContain("Anthropic's official CLI for Claude")
})

test('built-in agent prompts describe FreeCoding instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('FreeCoding')
  ...
  expect(generalPrompt).toContain('FreeCoding')
  ...
  expect(explorePrompt).toContain('FreeCoding')
  ...
  expect(guidePrompt).toContain('FreeCoding')
  expect(guidePrompt).toContain('You are the FreeCoding guide agent.')
  expect(guidePrompt).toContain('**FreeCoding** (the CLI tool)')
