/**
 * FreeCoding startup screen — filled-block text logo with sunset gradient.
 * Called once at CLI startup before the Ink UI renders.
 *
 * Addresses: https://github.com/evoluindocomia/affhub-freecoding/issues/55
 */
...
function detectProvider(): { name: string; model: string; baseUrl: string; isLocal: boolean } {
  const useGemini = isEnvTruthy(process.env.FREECODING_USE_GEMINI) || isEnvTruthy(process.env.CLAUDE_CODE_USE_GEMINI)
  const useGithub = isEnvTruthy(process.env.FREECODING_USE_GITHUB) || isEnvTruthy(process.env.CLAUDE_CODE_USE_GITHUB)
  const useOpenAI = isEnvTruthy(process.env.FREECODING_USE_OPENAI) || isEnvTruthy(process.env.CLAUDE_CODE_USE_OPENAI)
  const useMistral = isEnvTruthy(process.env.FREECODING_USE_MISTRAL) || isEnvTruthy(process.env.CLAUDE_CODE_USE_MISTRAL)
...
export function printStartupScreen(): void {
  ...
  out.push(`${rgb(...BORDER)}\u255a${'\u2550'.repeat(W - 2)}\u255d${RESET}`)
  out.push(`  ${DIM}${rgb(...DIMCOL)}AffHub FreeCoding ${RESET}${rgb(...ACCENT)}v${MACRO.DISPLAY_VERSION ?? MACRO.VERSION}${RESET}`)
  out.push('')
