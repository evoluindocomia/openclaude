...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'freecoding-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    '.freecoding/wiki/schema.md',
    '.freecoding/wiki/index.md',
    '.freecoding/wiki/log.md',
    '.freecoding/wiki/pages/architecture.md',
  ])
  expect(await readFile(paths.schemaFile, 'utf8')).toContain(
    '# FreeCoding Wiki Schema',
  )
  expect(await readFile(paths.indexFile, 'utf8')).toContain('Wiki')
  expect(await readFile(paths.logFile, 'utf8')).toContain(
    'Wiki initialized by FreeCoding',
  )
