...
```bash
git clone https://node.gitlawb.com/z6MkqDnb7Siv3Cwj7pGJq4T5EsUisECqR8KpnDLwcaZq5TPr/freecoding.git
cd freecoding
...
If you use the in-app provider wizard, choose `Codex OAuth` to open ChatGPT sign-in in your browser and let FreeCoding store Codex credentials securely.

If you already use the Codex CLI, FreeCoding reads `~/.codex/auth.json` automatically. You can also point it elsewhere with `CODEX_AUTH_JSON_PATH` or override the token directly with `CODEX_API_KEY`.
...
```bash
ollama launch freecoding --model llama3.3:70b
```
...
# launch using persisted profile (.freecoding-profile.json)
bun run dev:profile
